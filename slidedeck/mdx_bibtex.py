...
def bib_format_record(record):
    s = u""
    for entry in bib_formats.get(record['ENTRYTYPE'], []):
        value = record.get(entry[0], "")
        if value:
            s += str(entry[1]).format(str(value))
    s += "."
    return s
...
class BibtexExtension(markdown.Extension):
    ...
    def load_bib(self):
        ...
        if hasattr(self.md, 'Meta') and 'bibliography' in self.md.Meta:
            bibfile = os.path.join(self.getConfig(
                'root'), self.md.Meta['bibliography'][0])
        else:
    ...
    def references(self):
        ...
        return ('\n<footer class="source"><ul class="citation-references">\n'
